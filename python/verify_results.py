...
matmul (tb/matrix_mult_tb.sv):        layer (tb/nn_layer_tb.sv):
    ...
    dims M K N                            dims INPUT_SIZE OUTPUT_SIZE
    case <name>                           case <name>
...
from golden_model import layer_relu, matmul_int8
...
def verify_layer(path):
    (in_size, out_size), cases = parse_results(path, "layer")
    mismatches = []
    for index, case in enumerate(cases):
        y_rtl = case["Y"]
        y_ref = layer_relu(case["X"], case["W"].reshape(in_size, out_size), case["B"])
...
def main(argv):
    if len(argv) != 3 or argv[1] not in RECORD_KINDS:
        ...
        return 2
    try:
        return verify_matmul(argv[2]) if argv[1] == "matmul" else verify_layer(argv[2])
