...
//  WRITEBACK  | m_ready, more elements left    | COMPUTE   | next (i, j), row-major
...
//   m_valid   = WRITEBACK             m_last    = WRITEBACK and (i, j) = (M-1, N-1)
//   busy      = not IDLE              done      = DONE (one-cycle pulse)
//
// Cycles per job with no stalls (8x8x8 in brackets):
//   IDLE sees s_valid          1
//   LOAD                       M*K + K*N           [128]
//   COMPUTE + WRITEBACK        M*N * (K + 1)       [576]  K MAC cycles + 1 output beat each
//   DONE                       1
//   total                      M*K + K*N + M*N*(K+1) + 2   [706]
...
module matmul_ctrl #(
    ...
    output logic [IW-1:0] i,           // C element being computed: row
    output logic [JW-1:0] j,           // C element being computed: column
    output logic [KW-1:0] k            // term within its dot product
);
    ...
    logic          last_elem;      // (i, j) is the final element of C

    assign last_elem = (i == I_LAST) && (j == J_LAST);
        ...
        if (rst) begin
            ...
            case (state)
                ...
                S_COMPUTE:
                    k <= (k == K_LAST) ? '0 : k + KW'(1);
                S_WRITEBACK:
                    if (m_ready && !last_elem) begin   // on to the next element, row-major
                        if (j == J_LAST) begin
                            j <= '0;
                            i <= i + IW'(1);
                        end else begin
